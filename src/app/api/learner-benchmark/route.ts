...
import { readLearnerTierColumn } from '@/lib/sheets'

export async function GET(_req: NextRequest): Promise<NextResponse> {
  const rawCreds = process.env.GOOGLE_SERVICE_ACCOUNT_JSON
  const sheetId = process.env.GOOGLE_SHEET_ID
  if (!rawCreds || !sheetId) {
    console.error('[learner-benchmark] Server misconfiguration: missing env vars')
    return NextResponse.json({ error: 'Server misconfiguration' }, { status: 500 })
  }
  try {
    JSON.parse(rawCreds)
  } catch {
    console.error('[learner-benchmark] Server misconfiguration: GOOGLE_SERVICE_ACCOUNT_JSON is not valid JSON')
    return NextResponse.json({ error: 'Server misconfiguration' }, { status: 500 })
  }
  ...
  try {
    ...
    console.error('[learner-benchmark] Failed to read benchmark data:', err)
